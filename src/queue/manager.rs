...
impl QueueManager {
    ...
    async fn process_queue(&self) {
        ...
        for task in queue.iter_mut() {
            if task.status == TaskStatus::Downloading && !active.contains_key(&task.id) {
        ...
        for task in queue.iter_mut() {
            ...
            if task.status == TaskStatus::Queued {
                ...
                info!(
                    "🚀 [SCHEDULER] Atomically reserved slot for task: {}",
                    task.id
                );
