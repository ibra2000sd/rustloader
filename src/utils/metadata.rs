...
impl MetadataManager {
        ...
        let filtered: Vec<VideoMetadata> = all_metadata
            ...
            .filter(|m| {
                m.title.to_lowercase().contains(&query_lower)
                    || m.channel.as_ref().map_or(false, |c| c.to_lowercase().contains(&query_lower))
                    || m.uploader.as_ref().map_or(false, |u| u.to_lowercase().contains(&query_lower))
            })
            .collect();
    ...
    pub async fn toggle_favorite(&self, video_id: &str) -> Result<bool> {
        ...
        eprintln!("⭐ [METADATA] Toggled favorite for {}: {}", video_id, is_favorite);
        Ok(is_favorite)
    }
    ...
    pub async fn get_stats(&self) -> Result<MetadataStats> {
        ...
        for metadata in &all_metadata {
            ...
            *by_source.entry(metadata.source_platform.clone()).or_insert(0) += 1;
        }
